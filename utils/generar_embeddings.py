...
class GeneradorEmbeddingsTFLite:
    ...
    def procesar_usuarios(self):
        usuarios = sorted(os.listdir(self.faces_folder))
        if not usuarios:
            print(f"⚠️ Carpeta {self.faces_folder} vacía o sin usuarios.")
            return

        for usuario in usuarios:
            ruta_usuario = os.path.join(self.faces_folder, usuario)
            if os.path.isdir(ruta_usuario):
                print(f"Procesando usuario: {usuario}")
                embedding = self.generar_embedding_promedio(ruta_usuario)
                if embedding is not None:
                    ruta_salida = os.path.join(self.embeddings_folder, f"{usuario}.npy")
                    np.save(ruta_salida, embedding)
                    print(f"Embedding guardado: {ruta_salida}")
                else:
                    print(f"No se pudo generar embedding para: {usuario}")

    def generar_embedding_promedio(self, ruta_usuario):
        embeddings = []
        for i in range(1, 7):  # imágenes 1.jpg a 6.jpg
...
class RegistrarRostro(Screen):
    ...
    def on_leave(self):
        ...
        # Generar embeddings para todos los usuarios al salir
        try:
            ruta_modelo = os.path.join("models", "mobilefacenet.tflite")
            generador = GeneradorEmbeddingsTFLite(ruta_modelo)
            generador.procesar_usuarios()
        except Exception as e:
    ...
    def iniciar_captura_multiple(self, *args):
        ...
        Clock.schedule_once(lambda dt: self.mostrar_resultado_generacion(nombre), 7)
    ...
    def mostrar_resultado_generacion(self, nombre):
        ...
        if embedding is not None:
            ...
            self.label_mensaje.text = f"No se pudo generar embedding para {nombre}, repite la captura"
